...
def taylor(f,fsecond,t0,y0,h,n):
    T,Y = [],[]
    for i in range(n):
        if i == 0:
            T.append(t0)
            ...
            T.append(T[-1] + h)
            Y.append(Y[-1]+f(T[i-1],Y[i-1])*h + (fsecond / 2)*(h**2))
    return T,Y
...
def main():
    ...
    T,Y = taylor(F,t0,y0,0.1,n)
    T2,Y2 = taylor(F,t0,y0,0.01,n)
    T3,Y3 = taylor(F,t0,y0,0.001,n)
    T4,Y4 = taylor(F,t0,y0,0.0001,n)
