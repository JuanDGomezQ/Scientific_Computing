...
def centrado(x,h):
    """
        Esta funcion aplica la formual para derivacion mediante el metodo centrado
    """
    ans =(f(x + h) -f(x - h)) / (2*h)
    return ans
...
def pruebas(x,hs):
    ...
    """
    print()
    print(" Metodo Centrado:")
    print(" -------------------------------------------------------------------------")
    for i in hs:
        start = time()
        ans = centrado(x,i)
        elapsed = time() - start
        real = 24*pow(x,2) + 4*(x) + 2  #Derivada Analitica, colocar la derivada correspondiente a la funcion usada.
        error = abs(ans - real)
...
def main():
    """
        Funcion Principal que llama a los metodos para hacer diferencias por metodo centrado, despues grafica
    """
    x = 5
    h = 1
    ...
    plt.legend(loc = "upper right")
    plt.title("Metodo Centrado | f(x) = log(x**5 + 100) | f'(x) = 5x**(4)  / 2(x**(5) + 100) ")
    plt.grid()
    plt.show()
