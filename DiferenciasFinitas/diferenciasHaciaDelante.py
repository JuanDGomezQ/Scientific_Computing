...
def DifInfiDelante(x,h):
    """
        Esta funcion aplica la formual para derivacion finita hacia delante
    """
    ans =(f(x + h) - f(x)) / h
    return ans
...
def pruebas(x,hs):
    ...
    """
    print()
    print(" Metodo Hacia Delante:")
    print(" -------------------------------------------------------------------------")
    for i in hs:
        start = time()
        ans = DifInfiDelante(x,i)
        elapsed = time() - start
...
def main():
    """
        Funcion Principal que llama a los metodos para hacer diferencias hacia adelante, despues grafica
    """
    x = 5
    hs = [0.2,1,2,3]
    pruebas(x,hs)
    ...
    plt.legend(loc = "upper right")
    plt.title("Metodo Hacia Delante | f(x) = log(x**5 + 100) | f'(x) = 5x**(4)  / 2(x**(5) + 100) ")
    plt.grid()
    plt.show()
